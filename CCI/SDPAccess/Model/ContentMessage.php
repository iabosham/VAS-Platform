...
class ContentMessage extends DBConnection {
    ...
    public function updateContentMessageData($msg, $orderId,$sendDate,$userId, $id) {
        ...
        $isUpdated = false;
        try {
            $stmt = $this->connection->prepare("update message_content set msg=?,serial_id=? ,sending_date=? $extra "
                    . "  where id = ?")
            ;

            $stmt->bind_param("sisi", $msg, $orderId,$sendDate, $id);
            ...
            if ($stmt->affected_rows > 0) {
                ...
                General::addError($stmt->error);
                General::writeEvent("updateContentMessageData error" . $stmt->error);
            }
    ...
    public function getContentMessagesByClientIdData($clientId) {
        ...
        $qry = "select message_content.id,message_content.sending_date,message_content.status,message_content.serial_id,message_content.msg "
                ...
                . "where message_content.sub_service_id = sub_contents.id and sub_contents.content_id=contents.id and client_id = ? ";
