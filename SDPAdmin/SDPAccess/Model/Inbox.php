...
class Inbox extends DBConnection {
    ...
    public function getInboxMessages($companyId, $fromDate, $toDate, $shortcodeId) {

        $where = "";

        if ($shortcodeId > 0) {

            $where .= " and inbox.SHORTCODESTR = '$shortcodeId' ";
        }

        if ($companyId > 0) {

            $where .= " and smpp_connections.operator_id = $companyId ";
        }
        if ($fromDate != null) {
        ...
        $table = array();
        $stmt = $this->connection->prepare(" select inbox.*, smpp_connections.title as operatorName "
                . "from inbox,smpp_connections where inbox.CONNECTIONID = smpp_connections.id $where "
