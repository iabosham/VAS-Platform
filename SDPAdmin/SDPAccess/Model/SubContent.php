...
class SubContent extends DBConnection {
    ...
    public function getSubContentsData($contentId) {
        ...
        $qry = "select sub_contents.*,contents.title as contentTitle from sub_contents,contents"
                . " where sub_contents.content_id =contents.id $where order by id   ";
    ...
    public function getSubContentInfoByIdData($sub_contentsId) {
        ...
        if($sub_contentsId > 0){
            $where.= " where id = $sub_contentsId ";
            
        }
        
        $qry = "select * from sub_contents $where  ";
        $row = null;
        $result = $this->connection->query($qry);
        if ($result != null) {
            if ($result->num_rows > 0) {
     ...
     public  function deleteSubContentData($sub_contentsID){
       ...
       public function updateContentExtraIdData($contentExtraId,$id) {

        $isUpdated=false;
        try {

            $stmt = $this->connection->prepare("update sub_contents set content_extra_id = ? where id = ? ");
           
            $stmt->bind_param("ii",$contentExtraId,$id);
            ...
            if ($stmt->affected_rows > 0 || $stmt->affected_rows == 0  ) {
               $isUpdated = true ;
                ...
                General::writeEvent("updateContentExtraIdData1 error" . $stmt->error);
            }
            $stmt->close();
        } catch (Exception $e) {
            General::writeEvent("updateContentExtraIdData2 error" . $e->getMessage());
        }
