...
class TransDataSource extends DBConnection{
  ...
  function __construct() {
    ...
    public  function addTransData($id,$title ){
      $add_qry = "insert into transceivers (TRANSCID,TITLE) "
              . " values($id,'$title') " ;
   ...
   ///
        ...
        try {
            ...
            $stmt = $this->connection->prepare("insert into transcconnections(TRANSCID,CONNECTIONID) "
                    . " values(?,?) ");
     ...
     public function getConnectionsByTransIdData($transId) {
         $table = array();
        try {
            $stmt = $this->connection->prepare("select transcconnections.id, smppconnections.CONNECTIONID,"
                    . "smppconnections.TITLE from transcconnections,smppconnections where "
                    . "transcconnections.TRANSCID= ? "
                    . "and transcconnections.CONNECTIONID = smppconnections.CONNECTIONID  ");
    ...
    public function deleteConnectionData($id) {
        ...
        try {
            ...
            $stmt = $this->connection->prepare("delete from transcconnections where id = ?");
