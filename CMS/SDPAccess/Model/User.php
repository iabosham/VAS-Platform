...
class User extends DBConnection {
    ...
    //  Insert new user 
    public function addUserData($name, $login, $password, $type, $email, $phone, $sys) {
        $secret = md5($password);
        $add_qry = "insert into user (name,login,password,user_type,email,phone,system_id) "
                . " values('$name','$login','$secret','$type','$email','$phone',$sys) ";
    ...
    public function updateUserData($id, $name, $login, $type, $email, $phone, $status, $secret) {
        ...
        $upt_qry = "update user set name = '$name',login='$login',user_type='$type',email='$email',phone='$phone',status=$status  $sql where id = $id  ";
    ...
    public function checkLoginData($login, $password, $systemId =1) {
        $pass = md5($password);
        try {
