...
class Service extends DBConnection {
    ...
    public  function addServiceData($name, $serviceType, $serviceKey, $shortcodeID, $userID,$subMessage,$unSubMessage,$senderName,$freeSource,$method,$subServiceId){
      $serviceId = 0  ;  
    try {
      $stmt = $this->connection->prepare("insert into service (name,service_type_id,service_key,shortcode_id,"
              . "user_id,sub_message,unsub_message,sender_name,free_source,service_method,sub_service_id)"
              . " values(?,?,?,?,?,?,?,?,?,?,?)");
      
      $stmt->bind_param("sisiissssii",$name, $serviceType, $serviceKey, $shortcodeID, $userID,$subMessage,$unSubMessage,$senderName,$freeSource,$method,$subServiceId);
      $stmt->execute();
    ...
    public function updateServiceData($id,$name, $serviceType, $serviceKey, $shortcodeID, $userID,$subMessage,$unSubMessage,$senderName,$freeSource,$method) {
        $update_qry = "update service set name = '$name',service_type_id=$serviceType,service_key='$serviceKey',"
                ...
                . ",sender_name='$senderName',free_source='$freeSource',service_method='$method' "
                . "where id =  $id " ;
    ...
    public function getServicesData() {
        $qry = "select service.id,service.name as serviceName,service.service_code,service.service_key,"
                . "vendor.name as vendorName,"
                . "shortcode.number,service_type.name as serviceTypeName "
                . "from service,service_type,shortcode,vendor where service.shortcode_id=shortcode.id and "
    ...
    public function getServiceInfoByIdData($id) {
        $qry = "select service.id, service.name,service.sender_name,service.free_source,service.service_method,service.sender_name,service.comment,service.service_key,service.service_code,"
                . "service.isActive,service.creationDate,service.sub_message,service.unsub_message,service.service_type_id,service.shortcode_id ,"
                ...
                . "and service.sub_service_id = sub_contents.id and sub_contents.content_id=contents.id   ";
