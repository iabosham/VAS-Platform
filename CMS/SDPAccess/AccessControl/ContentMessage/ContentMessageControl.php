...
class ContentMessageControl extends ContentMessage{
    ...
    public static function getContentMessagesByClientId($from,$to,$clientId){
       
        $contentTypeObj = new ContentMessage();
        return $contentTypeObj->getContentMessagesByClientIdData($from,$to,$clientId);
    }
    
    public static function getInfoByDateAndOrder($serviceId, $order, $date){
       
        $contentTypeObj = new ContentMessage();
        return $contentTypeObj->getInfoByDateAndOrderData($serviceId, $order, $date);
    }
  ...
  public static function AddContentMessage($contentInfo){
   ...
   public static function updateContentMessage($msg, $orderId, $sendDate, $userId, $id,$status){
       $contentTypeObj = new ContentMessage();
       $result = $contentTypeObj->updateContentMessageData($msg, $orderId, $sendDate, $userId, $id,$status);
       $contentTypeObj->close();
   return $result;
   }
